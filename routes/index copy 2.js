...
BatchR.find({refNumber:refNumber},function(err,loc){
  ...
  BatchR.findByIdAndUpdate(batchId,{$set:{dispatchDate:date,truck:truck,salesPerson:salesPerson,
  time:time,destination:destination,dispatcher:dispatcher,casesBatch:cases,dateValueDispatch:dateValue,dispatchMformat:mformat,salesPerson:salesPerson}},function(err,kocs){
      ...
      product:product,refNumber:refNumber,destination:destination,batchId:batchId  }},function(err,docs){
  ...
  router.get('/dispatchStock/:id',isLoggedIn,function(req,res){
    ...
    var refNumber = req.user.refNumDispatch
...
StockV.find({refNumDispatch:refNumber},function(err,focs){
  let size  = focs.length + 1
...
res.send(c)
        ...
        let tCases = hoc.cases + 1
            ...
            mformatDispatch:mformat,size:size,casesDispatched:1}},function(err,lof){
