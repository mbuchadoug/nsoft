...
var ZApp = (function(Framework) {
    ...
    try {
        ...
        function _LoadExtension(extensionData) {
            ...
            try {
                ...
                if (typeof extData !== 'object') {
                    Logger.Error('Invalid extension config passed');
                    return;
                }
        ...
        function getFilteredConfigParams(configArr) {
            if (isDevMode()) {
                return configArr;
            } // If in Devmode all config params are needed for manipulation.

            if (Array.isArray(configArr)) {
                return configArr.filter(function(config) {
                    return !Boolean(config.secure);
                });
            }
        }
        ...
        function FetchLocaleResource(extensionID) {
            return new Promise(function(resolve, reject) {
                ...
                if (extensionData && extensionData.locale === undefined) {
                    ...
                    resolve({
                        locale: extensionData.locale,
                        content: extensionData.localeResource
                    });
                } else {
