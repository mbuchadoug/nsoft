...
ZCMediaConstants = {
    ...
    groupcallThemeSettingsHtml: '<div id="groupcall_theme_config_cnt" class="zcl-card smartconf-themes-settings" purpose="card" type="conference_themes">\n\t\t\t<div class="zcl-card-head">{{theme_hdr}}</div>\n\t\t\t<div id="smartconf_theme_mandate_banner" class="orgLockedDetail flexC curD dN">\n\t\t\t\t<div class="flexM orgLockIconCnt fshrink h100">\n\t\t\t\t\t<span class="clr-warning font17 zcf-lock"></span>\n\t\t\t\t</div>\n\t\t\t\t<div class="clr-S ellips font14 pLR10 w100">{{locked_info}}</div>\n\t\t\t</div>\n\t\t\t<div class="zcl-card-body flexC gap15">\n\t\t\t\t<div class="font14 fontB">{{sel_theme}}:</div>\n\t\t\t\t<div id="smartconf_themes_settings_drpdwn_cnt" class="smartconf-themes-settings-drpdwn">\n\t\t\t\t\t<span class="skelton-loader form-skelton flex"></span>\n\t\t\t\t</div>\n\t\t\t</div>\n\t\t\t<div class="zcl-tip-box ftr-tip">\n\t\t\t\t<span class="zcf-hint zcl-tip-icon flexM"></span>\n\t\t\t\t<span class="zcl-tip-text flexC">\n\t\t\t\t\t<span><b>{{note_hdr}}:</b> {{note}}</span>\n\t\t\t\t</span>\n\t\t\t</div>\n\t\t</div>',
    themeDropDownCntHtml: '<div class="zc-av-zcl-dropdown-input pLR10" mediamodulebuttons purpose="openConferenceThemesSettingDropDown">\n\t\t\t<span class="drpdwn-label ellips">{{drpdwn_label}}</span>\n\t\t\t<span class="srchin-downArrow zcf-downArrow"></span>\n\t\t</div>',
    themeDropDownHtml: '<div id="smartconf_themes_settings_drpdwn" class="zc-av-zcl-menu-wrap no-pointer p10 w100" align="top" style="display: none;">\n\t\t\t<div class="zcl-search2 mB15 {{search_hide_cls}}">\n\t\t\t\t<div class="zcf-search2 zcl-seach-icon font15"></div>\n\t\t\t\t<div class="zcl-search-input">\n\t\t\t\t\t<input type="text" placeholder="{{search_ph}}" mediamoduleinput purpose="handleConferenceThemesListSearch">\n\t\t\t\t</div>\n\t\t\t</div>\n\t\t\t<div id="smartconf_themes_settings_drpdwn_items" class="smartconf-themes-settings-drpdwn-items flex flexW gap10 ovrflwA">{{drpdwn_items_html}}</div>\n\t\t</div>',
    themeDropDownItem: '<div class="smartconf-themes-settings-item flex-col">\n\t\t\t<img id="{{item_id}}" class="smartconf-themes-settings-item-thumb {{sel_cls}}" src="{{thumbnail_url}}" mediamodulebuttons purpose="setPreferredConferenceTheme">\n\t\t\t<span class="font12 line22 textC ellips">{{theme_name}}</span>\n\t\t</div>',
    ...
    getConferenceThemesSettingHtml: function() {
        return $WC.template.replace(this.groupcallThemeSettingsHtml, {
            ...
            $note: "media.recording.settings.note.user_choice"
        })
    },
    updateConferenceThemesSettingDropDown: function() {
        ZCSmartConferenceThemesUI.getBasicDetailsOfThemes().then(e => {
            let t = e.find(e => "mandate" === e.status),
                i = !0;
            if ($WC.Util.isEmpty(t)) {
                i = !1;
                let n = MediaUtil.getSettingsValue("conference_theme", !0);
                t = e.find(e => e.id === n), $WC.Util.isEmpty(t) && (t = e.find(e => "default" === e.status))
            }
            let n = "";
            e.forEach(e => {
                n += $WC.template.replace(this.themeDropDownItem, {
                    item_id: e.id,
                    sel_cls: e.id === t.id ? "sel" : "",
                    thumbnail_url: e.thumbnail_url,
                    theme_name: e.name
                })
            }), n = $WC.template.replace(this.themeDropDownHtml, {
                drpdwn_items_html: n
            }, "InSecureHTML"), n = $WC.template.replace(n, {
                search_hide_cls: e.length > 9 ? "" : "dN",
                $search_ph: "avcliq.conf.themes.search"
            });
            let a = $WC.template.replace(this.themeDropDownCntHtml, {
                drpdwn_label: t.name
            });
            ZCJQuery("#smartconf_theme_mandate_banner").toggleClass("dN", !i), ZCJQuery("#smartconf_themes_settings_drpdwn_cnt").html(a + n).toggleClass("zcdisabled", i)
        })
    },
    getCallSettingsHtml: function(e) {
        var t = this.getGroupCallPersonalSettingsHtml(e);
        return ZCAVCP.isEnabledForMeeting(ZCAVCP.properties.meeting.SHOW_RECORDING_SETTINGS) && (t += this.getGroupCallRecordingSettingsHtml(e)), Conference.isVideoEffectsSettingsEnabled() && e && (this.showPreferredDeviceSettings(), t += this.getVideoEffectsSettingHtml(e)), Conference.isThemesEnabled() && e && (t += this.getConferenceThemesSettingHtml(), this.updateConferenceThemesSettingDropDown()), t
    },
    ...
    getGroupCallRecordingSettingsHtml: function(e) {
        var t = SecurityManager.getModuleConfig("group_call_recording", "allowed_group_call_recording"),
            ...
            n = "media.recording.settings.note.user_choice";
    ...
    UIEvents: {
        ...
        openConferenceThemesSettingDropDown: function(e, t) {
            let i = t.parents("#smartconf_themes_settings_drpdwn_cnt");
            Clickoutside.bind({
                srcid: i.attr("id"),
                destid: "smartconf_themes_settings_drpdwn",
                customHide: e => {
                    i.find('[mediamodulebuttons][purpose="openConferenceThemesSettingDropDown"]').removeClass("sel"), ZCJQuery("#" + e.destid).hide()
                },
                customShow: e => {
                    i.find('[mediamodulebuttons][purpose="openConferenceThemesSettingDropDown"]').addClass("sel"), ZCJQuery("#" + e.destid).show()
                }
            })
        },
        setPreferredConferenceTheme: function(e, t) {
            Clickoutside.handleClickOnChild(e);
            let i = e.target.id;
            MediaUtil.updateSettingsValue({
                conference_theme: i
            }), MediaTemplates.updateConferenceThemesSettingDropDown()
        },
    ...
    UIInputEvents: {
        handleConferenceThemesListSearch: function(e, t) {
            let i = t[0].value.trim().toLowerCase();
            ZCSmartConferenceThemesUI.getBasicDetailsOfThemes().then(e => {
                let t = e.find(e => "mandate" === e.status);
                if ($WC.Util.isEmpty(t)) {
                    let i = MediaUtil.getSettingsValue("conference_theme", !0);
                    t = e.find(e => e.id === i), $WC.Util.isEmpty(t) && (t = e.find(e => "default" === e.status))
                }
                let n = "";
                e.forEach(e => {
                    e.name.toLowerCase().includes(i) && (n += $WC.template.replace(MediaTemplates.themeDropDownItem, {
                        item_id: e.id,
                        sel_cls: e.id === t.id ? "sel" : "",
                        thumbnail_url: e.thumbnail_url,
                        theme_name: e.name
                    }))
                }), "" === n && (n = $WC.template.replace('<div class="flexM w100"><span class="p10">{{not_found}}</span></div>', {
                    $not_found: "common.search.result.none"
                })), ZCJQuery("#smartconf_themes_settings_drpdwn_items").html(n)
            })
        }
    },
    UIToggleEvents: {
    ...
    return "undefined" != typeof $ZCUtil && $ZCUtil.isJQueryAvailable() && void 0 === window.ZCJQuery && (window.ZCJQuery = window.jQuery), {
        ...
        bindEvents: function() {
            if (!t) {
                ...
                })), ZCJQuery(document).on("input", "[mediamoduleinput]", (function(e) {
                    e.stopPropagation();
                    var t = ZCJQuery(this),
                        i = t.attr("purpose");
                    MediaHandler.UIInputEvents[i](e, t)
...
MediaAPI = {
    ...
    getBasicDetailsOfThemes: function(e, t) {
        MediaAPI.request({
            url: "/v2/mediathemes?type=conference",
            type: "GET",
            success: t => e instanceof Function && e(t.data),
            error: e => t instanceof Function && t(e.data)
        })
    },
...
ConferenceLog = {
    ...
    statsGatheringIntervalTime: void 0,
    meetingStatsGatheringIntervalTime: void 0,
    statsCanBePushed: !1,
    ...
    connectionStats: {
        ...
        getAnalyticsObject: function(e) {
            let t = {},
                i = 1;
            for (let n in e) {
                if (i % (ConferenceLog.meetingStatsGatheringIntervalTime / 1e3) == 0) {
                    let i = e[n];
                    t[n] = i
                }
                i += 1
        ...
        isAnalyticsObjectLimitReached: function() {
            for (let e of ConferenceLog.connectionStats.analyticsData)
                if (30 === Object.keys(e.data).length) return !0;
            return !1
        },
        ...
        callBack: function(e, t, i, n, a) {
            if (ConferenceLog.isStatsCanBePushed() && (ConferenceLog.connectionStats.filterActiveVideoSSRCStats(i, a), ConferenceLog.connectionStats.data.push(MediaUtil.WebRTCStats.getConnectionStatsPayload(i, a)), ConferenceLog.connectionStats.connectionIds.delete(e), 0 === ConferenceLog.connectionStats.connectionIds.size)) {
                ...
                if (ConferenceLog.meetingStatsGatheringIntervalTime >= 5e3) ConferenceAPI.pushConnectionStats(a.conferenceId, e);
                else {
                    for (let t of e) {
                    ...
                    }(a.lastChunk || ConferenceLog.connectionStats.isAnalyticsObjectLimitReached()) && (ConferenceAPI.pushConnectionStats(a.conferenceId, ConferenceLog.connectionStats.analyticsData), ConferenceLog.connectionStats.analyticsData = [])
                }
                ConferenceLog.connectionStats.data = [], ConferenceLog.connectionStats.connectionIds = new Set(WebRTCPeerConnectionStats.getConnectionIds("conferences"))
            }
    ...
    updateWebRTCStatsConfiguration: function(e) {
        this.clearWebRTCStatsData(), $WC.Util.isEmpty(e.stats_gathering_interval_time) || (ConferenceLog.statsGatheringIntervalTime = parseInt(e.stats_gathering_interval_time)), $WC.Util.isEmpty(e.stats_can_be_pushed) || (ConferenceLog.statsCanBePushed = "true" == e.stats_can_be_pushed), $WC.Util.isEmpty(e.objects_to_push_via_websocket) || (ConferenceLog.statsObjectMaxSize = parseInt(e.objects_to_push_via_websocket)), $WC.Util.isEmpty(e.meeting_stats_gathering_interval_time) || (ConferenceLog.meetingStatsGatheringIntervalTime = parseInt(e.meeting_stats_gathering_interval_time))
    },
    wmsMsgTypes: {
        START: "start",
            ...
            if (ConferenceLog.writeToLog(ConferenceLog.constants.UIEvents.startTriggered, o), this.isInitialized())
                ...
                Conference.isVideoConference(e) ? Conference.isVideoConferenceAllowed() || (e = Conference.types.AUDIO, ConferenceLog.writeToLog(ConferenceLog.constants.errorEvents.videoConferenceNotAllowed)) : Conference.isAudioConferenceAllowed() || (e = Conference.types.VIDEO, ConferenceLog.writeToLog(ConferenceLog.constants.errorEvents.audioConferenceNotAllowed)), ConferenceLog.writeToLog(ConferenceLog.constants.aysncEvents.existingSessionCheck), MediaManager.handleExistingSessions((function() {
                    if (ConferenceLog.writeToLog(ConferenceLog.constants.aysncEvents.existingSessionCheckSuccess), !MediaManager.isCustomRequestPending(ZCMediaConstants.requests.startTrigger)) {
                        ...
                        ConferenceLog.writeToLog(ConferenceLog.constants.aysncEvents.syncPreferredDevices), MediaManager.setCustomRequestAsPending(ZCMediaConstants.requests.startTrigger), s && s.isQuickStart ? ZCSmartConferenceImpl.prefetchMetaDetails(a, a) : MediaManager.prefetchMetaDetails(a)
                    }
                }))
...
void 0 !== MediaUtil && MediaUtil.isAVLibraryLoadedInCliq() && (window.cssPath = void 0, window.jsPath = void 0, window.skipZSWmsLiteImpl = !1, window.doNotExtendRemoveClass = !1), (Presentation = function(e) {
    ...
    navigateToNextSlide: function() {
        zsSlideShow.set.next.call(zsSlideShow)
    },
    navigateToPreviousSlide: function() {
        zsSlideShow.set.previous.call(zsSlideShow)
    },
    ...
    _templates: {
        ...
        presentationViewerOptions: '<div purpose="jumpToHostView" mediamodulebuttons class="zc-av-presentation-option flexC font14 cur">\n\t\t\t\t\t\t\t\t\t\t\t      <div class="zcf-presenter cur font18"></div>\n\t\t\t\t\t\t\t\t\t\t\t      <div>{{goto_hostview}}</div>\n\t\t\t\t\t\t\t\t\t </div>'
    },
    ...
    handlePresentationSlideChange: function(e, t) {
        if (!t.isPresenter()) {
            ...
            PresentationImpl.isTimeEligibleToUpdateSlide(s) && (r == o && (zsSlideShow.set.jumpTo.call(zsSlideShow, {
                slideNo: n,
                animIndex: a
            }), PresentationImpl.setLastSlideUpdatedTime(s)), i.setPresenterSlideNo(n), i.setPresenterAnimIndex(a), PresentationUI.replaceHostIndicator(PresentationUI.getContainer().find("#slidecontainer_leftnav_cnt"), i.getPresenterSlideNo()))
        }
    },
    ...
    jumpToPresentationSlide: function(e, t, i) {
        ...
        t = void 0 !== t ? t : parseInt(e.val()) - 1, i = void 0 !== i ? i : PresentationImpl.getDefaultAnimIndex(), n.getPresentation().jumpToSlide(t, i)
    }
}, PresentationAPI = {
